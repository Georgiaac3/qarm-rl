...
def plot_comparison(ik_results: dict, rl_results: dict, save_path: str = None):
    ...
    ax = axes[0]
    ax.boxplot([ik_results["distances"], rl_results["distances"]],
               labels=["IK Only", "IK + RL"])
...
def visualize_trajectory(
    ...
    circle = patches.Circle((0, 0), env.arm.l1 + env.arm.l2,
                           fill=False, linestyle='--', color='gray', alpha=0.3)
    ax.add_patch(circle)
    
    # Plot arm in final configuration
    q_final = q_history[-1]
    elbow_pos = np.array([env.arm.l1 * np.cos(q_final[0]),
                          env.arm.l1 * np.sin(q_final[0])])
    ...
    ax.text(0.5, -0.95, f"Final distance: {final_distance:.4f} m",
...
def main():
    ...
    print("=" * 80)
    print("\nIK Only (Baseline):")
    print(f"  Mean distance: {ik_results['mean_distance']:.4f} m (±{ik_results['std_distance']:.4f})")
    print(f"  Success rate:  {ik_results['success_rate']*100:.1f}%")
    
    print("\nIK + RL (Hybrid):")
    print(f"  Mean distance: {rl_results['mean_distance']:.4f} m (±{rl_results['std_distance']:.4f})")
    ...
    # Comparison plot
    plot_comparison(ik_results, rl_results,
                   save_path=str(save_dir / "comparison.png"))
    
    # Trajectories
    print("\nVisualizing trajectories...")
    
    visualize_trajectory(eval_env, model=None, use_rl=False, num_steps=100,
                        save_path=str(save_dir / "trajectory_ik_only.png"))
    
    visualize_trajectory(eval_env, model=model, use_rl=True, num_steps=100,
