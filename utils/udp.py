...
def setup_udp_socket() -> socket.socket:
    ...
    logger.info(
        f"Socket UDP configuré: écoute sur {settings.local_bind_ip}:{settings.udp_port}, "
        f"envoi vers {settings.udp_ip}:{settings.udp_port}"
    )
    return sock
...
def send_control_commands(sock: socket.socket, velocities: List[float], grip_state: int) -> None:
    """
