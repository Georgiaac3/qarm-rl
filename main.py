...
def calculate_joint_velocities(current_time: float, angles: JointAngles) -> Tuple[List[float], int]:
    """
...
def main() -> None:
    ...
    """
    logger.info(
        f"Durée de simulation: {settings.simulation_duration}s, " f"Timestep: {settings.timestep}s"
    )
