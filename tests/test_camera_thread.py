...
def test_camera_connection():
    ...
    try:
        ...
        while time.time() - start_time < 10:  # Run for 10 seconds
            ...
            if frames is not None:
                ...
                if detection is not None and not detection_found:
                    ...
                    logger.info(f"Object detected at pixel ({u}, {v}) with confidence {confidence:.2f}")
        ...
        logger.info(f"✓ Camera thread alive: {robot.camera.camera_thread.is_alive() if robot.camera.camera_thread else False}")
